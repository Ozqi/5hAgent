...
- [ ] 上下文压缩, compact文件夹, 实现精细的压缩管理
  - [x] 已接入手动 `/compress` 基础链路，当前由并行 Agent 继续完善 task/tool 压缩入口。
  - [ ] 工具调用可能返回超长的返回值，网络请求可能返回超大文件。

- [x] history task
  - [x] task 完成后可通过 `/task archive <id>` 归档为 `history/*.md`，并在 `task.md` 原位置保留 `status/summary/history` 瘦身条目。
  - [x] 已支持 `/task reopen <id> [status]` 从 history 恢复回 active task。
  - [ ] 后续与 `/compress task <id>` 打通，让 `Compressed Context` 从 `pending` 切换为真实压缩引用。
