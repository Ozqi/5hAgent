// tasklist.go - 任务列表管理
// 功能：Task CRUD、状态流转、Markdown 持久化、历史归档
// 主要类型：Task, TaskList, TaskStatus, historyTaskSnapshot
// 导出函数：NewTaskList, CreateTask, UpdateTaskStatus, GetTask, ListTasks, DeleteTask, ArchiveTask, ReopenTask, GetProgress, ParseTaskStatus
package task
...
func (l *TaskList) ArchiveTask(id string) (*Task, error) {
	...
	historyPath := filepath.ToSlash(filepath.Join("history", replacer.Replace(task.ID)+".md"))
...
func (l *TaskList) saveLocked() error {
	...
	if start >= 0 && end >= start {
		...
		updated = strings.TrimRight(trimmed[:start], "\n") + "\n\n" + section + "\n"
	} else if trimmed == "" {
		updated = section + "\n"
	} else {
		updated = trimmed + "\n\n" + section + "\n"
