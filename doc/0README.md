...
> 由 Claude Fable 5 于 2026-08-23 阅读 `cmd/walle/*.go`、`internal/{runtime,agent,context,tools,agentd,tui}/*.go` 与 `doc/` 模块文档后更新。
...
flowchart TB
  ...
  main -->|ps / attach| agentd[internal/agentd]
  main --> tui[internal/tui]
  tui <-->|Unix Socket / NDJSON| agentd
...
| Agentd | [runtime/agent-agentd.md](runtime/agent-agentd.md) | 通用 process 调度与 Unix Socket 控制面 |
...
- 默认 `walle` 自动启动或复用用户级 `walle daemon`，然后为当前 workspace 打开新的 interactive Runtime；`-c/--continue` 才续接最近 Runtime 或 session。
- `walle daemon` 是用户级 supervisor，按 open 请求托管多个可 attach 的交互 Runtime；没有 `--poll` 或 `--interactive` 模式。
- `ps`、`attach`、`stop` 与 `internal/agentd` 的通用 process 能力保留。
