...
选择 Codex 后，daemon 发出可复制的登录链接事件，TUI 只负责展示并按事件提示打开浏览器。登录完成后 daemon 自动切换 provider，并发出模型选择器事件：
...
- 浏览器打不开不影响流程，daemon 始终通过事件给 TUI 一个可复制 URL。
...
第一版包括 daemon 侧浏览器 OAuth、刷新、模型目录、文本/推理流、工具调用和 TUI 展示的 picker 事件。不实现 logout、多账号轮换、额度仪表盘或外部 proxy。
